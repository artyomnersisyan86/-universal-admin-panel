...
export function SliderProps({ block, onPatch }: Props) {
  ...
  function handleAddSlide() {
    ...
    onPatch((b) => ({
      ...b,
      props: { ...b.props, slides: [...(b.props.slides ?? []), newSlide] },
    }));
  ...
  function handleRemoveSlide(id: string) {
    onPatch((b) => ({
      ...b,
      props: { ...b.props, slides: (b.props.slides ?? []).filter((s) => s.id !== id) },
    }));
  ...
  function handleMoveSlide(index: number, direction: 'up' | 'down') {
    ...
    onPatch((b) => {
      const next = [...(b.props.slides ?? [])];
      [next[index], next[targetIndex]] = [next[targetIndex], next[index]];
      return { ...b, props: { ...b.props, slides: next } };
    });
  }
  ...
  function updateActiveSlide(patch: Partial<Slide>) {
    ...
    onPatch((b) => ({
      ...
      props: {
        ...b.props,
        slides: (b.props.slides ?? []).map((s) =>
          s.id === activeSlideId ? { ...s, ...patch } : s,
        ),
      },
    }));
  }
